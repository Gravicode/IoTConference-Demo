    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            if (context.Request.InputStream.Length > 0)
            {
                ...
                string fName = "photo_"+ DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss")+".jpg";
                string FullPath =  context.Server.MapPath("~")+"/uploads/"+ fName;
                File.WriteAllBytes(FullPath,fileData);
                Url = "/uploads/"+fName;
            }
            var hasil = new Hasil() { Result = true, Keterangan = Desc, Url=Url };
